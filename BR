...
BloodyRevenge Official links 🌏

🌐 | Website (https://BloodyRevenge.io/)
🗒 | Certik Audit Processing  (https://www.certik.org/projects/BloodyRevenge)
📣 | Announcements (hhttps://t.me/BloodyRevenge_Channel)
🕊️ | Twitter (https://twitter.com/BloodyROfficial)
💬 | Telegram (https://t.me/BloodyRevenge_Global)
...
contract BR is ERC20PresetMinterPauser {
    ...
    uint256 public constant MAX_SUPPLY = 16000 * (10 ** 12) * (10 ** 18);
    uint256 public constant LOCK_BOT_DAYS = 6;
    
    
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "BR: ADMIN role required");
        _;
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, _msgSender()), "BR: MINTER role required");
        _;
    }

    modifier onlyLocker() {
        require(hasRole(LOCKER_ROLE, _msgSender()), "BR: LOCKER role required");
        _;
    }
    
    constructor() ERC20PresetMinterPauser("Bloody Revenge", "BR") {
    ...
    function mint(address to, uint256 amount) public virtual override onlyMinter {
        require(totalSupply() + amount <= MAX_SUPPLY, "BR: Max supply exceeded");
        _mint(to, amount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (_listBot[from].locked) {
            ...
            require(
                ...
                "BR: bot cannot transfer locked balance"
            );
        }
    }
    ...
    function withdrawERC20(address token, uint256 amount) external onlyAdmin {
        require(amount > 0, "BR: Amount must be greater than 0");
        require(IERC20(token).balanceOf(address(this)) >= amount, "BR: ERC20 not enough balance");
        require(IERC20(token).transfer(_msgSender(), amount), "BR: transfer ERC20 failed");
    }
